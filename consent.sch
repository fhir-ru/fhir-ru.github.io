...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2">
  ...
  <sch:pattern>
    ...
    <sch:rule context="f:Consent">
      ...
      <sch:assert test="exists(f:patient) or not(exists(f:scope/f:coding[f:system/@value='something' and f:code/@value='adr'])))">ppc-4: Если Scope=adr, должен быть пациент</sch:assert>
      <sch:assert test="exists(f:patient) or not(exists(f:scope/f:coding[f:system/@value='something' and f:code/@value='treatment'])))">ppc-5: Если Scope=treatment, должен быть пациент</sch:assert>
      <sch:assert test="exists(f:patient) or not(exists(f:scope/f:coding[f:system/@value='something' and f:code/@value='patient-privacy'])))">ppc-2: Если Scope=privacy, должен быть пациент</sch:assert>
      <sch:assert test="exists(f:patient) or not(exists(f:scope/f:coding[f:system/@value='something' and f:code/@value='research'])))">ppc-3: Если Scope=research, должен быть пациент</sch:assert>
      <sch:assert test="exists(f:policy) or exists(f:policyRule)">ppc-1: Policy или PolicyRule</sch:assert>
    </sch:rule>
